...
FILELIST_URL="$REPO_BASE/filelist_android.txt"
...
download_files() {
  ...
  # Download filelist_android.txt
  if ! curl -s "$FILELIST_URL" -o "$BOT_DIR/filelist_android.txt"; then
  ...
  done < "$BOT_DIR/filelist_android.txt"
}
